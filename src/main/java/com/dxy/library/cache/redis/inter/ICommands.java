...
public interface ICommands {
     ...
     * 自定义操作方法
     */
    void execute(CommandsConsumer consumer);

    /**
     * 自定义操作方法
     */
    <T> T execute(CommandsSupplier<T> supplier);

    /**
     * 自定义操作方法
     */
    <T> T execute(RedisFunction<JedisCommands, T> function);

    /**
     * 单机 / 主从模式的自定义操作方法
     */
    void single(RedisConsumer<Jedis> consumer);

    /**
     * 单机 / 主从模式的自定义操作方法
     */
    <T> T single(RedisFunction<Jedis, T> function);

    /**
     * 哨兵模式的自定义操作方法
     */
    void sentinel(RedisConsumer<Jedis> consumer);

    /**
     * 哨兵模式的自定义操作方法
     */
    <T> T sentinel(RedisFunction<Jedis, T> function);

    /**
     * 分片模式的自定义操作方法
     */
    void sharded(RedisConsumer<ShardedJedis> consumer);

    /**
     * 分片模式的自定义操作方法
     */
    <T> T sharded(RedisFunction<ShardedJedis, T> function);

    /**
     * 集群模式的自定义操作方法
     */
    void cluster(RedisConsumer<JedisCluster> consumer);

    /**
     * 集群模式的自定义操作方法
     */
    <T> T cluster(RedisFunction<JedisCluster, T> function);
     ...
     */
    Map<String, Long> del(String... keys);

    /**
     * 批量删除key，返回删除的 key 的数量
     */
    Map<String, Long> del(List<String> keys);
     ...
     * @param time 过期时间
     */
    <T> String set(String key, T value, String nxxx, String expx, long time);
     ...
     * @param time 过期时间
     */
    <T> String set(String key, T value, String expx, long time);
     ...
     * @param nxxx 判断操作在键存在或者不存在时进行，NX-只在键不存在时才对键进行设置操作，XX-只在键已经存在时， 才对键进行设置操作
     */
    <T> String set(String key, T value, String nxxx);
     ...
     */
    <T> Map<String, String> mset(Map<String, T> map);

    /**
     * 为批量的 key 设置值及其过期时间（单位为秒），返回OK表示成功
     */
    <T> Map<String, String> msetex(int seconds, HashMap<String, T> map);

    /**
     * 为批量的 key 设置值及其过期时间（单位为秒），返回OK表示成功
     */
    <T> Map<String, String> msetex(long time, TimeUnit timeUnit, HashMap<String, T> map);
